...
def spatial_inflow_score_single_interaction(
    adata,
    spatial_key: str,
    interaction: str,
    obs_key: str,
    ...
    colorbar_layout: str | None = 'side',  # 'side', 'bottom', or None
    ...
    percentile_scaling: tuple[int, int] | None = None,  # Tuple (low, high) e.g., (5, 95) to clip outliers
    vmin: float | None = None,  # Minimum value for color scale (overrides normalize)
    vmax: float | None = None,  # Maximum value for color scale (overrides normalize)
    shared_colorscale: bool = False,  # Use same scale for all cell types
    min_expression: float | None = None,  # Filter cells below this threshold
    show_counts: bool = True,  # Show cell counts in colorbar labels
    title: str | None = 'auto',  # 'auto' for auto-generate, None to hide, or custom string
    bbox: tuple[float, float, float, float] | None = None,  # (xmin, xmax, ymin, ymax) to zoom
    save_path: str | None = None,  # Path to save figure
    ...
    for label in labels:
        ...
        if not mask.any():
            warnings.warn(f"No cells found for cell type: {label}", stacklevel=2)# NOTE: this should also be with _logg
            continue
        ...
        if adata.raw is not None and interaction in adata.raw.var_names:
            ...
            if interaction not in adata.var_names:
                warnings.warn(f"Interaction '{interaction}' not found in adata.raw or adata.X. Skipping cell type {label}.", stacklevel=2)
                continue
        ...
        if len(expr_type) == 0:
            warnings.warn(f"No cells for {label} after filtering", stacklevel=2)
            continue
    ...
    if colorbar_layout == 'bottom':
        ...
        side_cbar_config = {'gs': gs, 'cbar_cols': cbar_cols, 'cbar_rows': cbar_rows}
    else:
